...
def github_webhook():
    ...
    if event == "pull_request":
        ...
        # Send an Issue comment to notify push events
        comment_url = f"https://api.github.com/repos/{repo_name}/issues/comments"
        headers = {"Authorization": f"token {token}", "Accept": "application/vnd.github.v3+json"}
        comment_data = {"body": f"🚀 New push detected by {pusher}!\nLatest commit message: {latest_commit}"}
        response = requests.post(comment_url, headers=headers, json=comment_data)

        if response.status_code == 201:
            print("✅ Push review success!")
        else:
            print(f"❌ Push comment failed: {response.status_code}, {response.text}")
